...
const Header  = ()=> {
  ...
  name.text(state.creator1.name);
  divInfoTitle.text(state.creator1.name);
  img.attr("src", state.user.image["60x60"].url);
  var pin = state.creator1.counts.pins;
  var seguidor = state.creator1.counts.followers;
  if(pin==1){
